...
<style>
    ...
    <section class="section">
        <form name=frm id=frm action="/admin/brands/save" method=post enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <?php
                    if(!empty($ufile1)){
                ?>
                    <a href="<?=!empty($ufile1) ? "/public/uploads/brands/{$ufile1}" : null?>" download="<?=$rfile1?>">
                        <img src="<?=!empty($ufile1) ? "/public/uploads/brands/{$ufile1}" : null?>" style="width: 100px; height: 100px; object-fit: cover; margin-top: 10px;">
                    </a>
                    <br>
            ...
            $.ajax({
                ...
                , success : function(response, status, request) {
                    if (response.result == true) {
                        ...
                        if('<?=$c_idx?>' != ""){
                            location.href="/admin/brands/write?c_idx="+ response.c_idx +"&s_parent_code_no=" + response.parent_code;
                        }else {
                            location.href="/admin/brands/list?s_parent_code_no=" + response.parent_code;
                        }
                        return;
                    } else {
                        alert(response.message);
