...
class CarBrandsController extends BaseController
{
    protected $carBrand;

    private $uploadPath = FCPATH ."public/uploads/brands/";

    public function __construct()
    {
        $this->carBrand = model("CarBrands");
    }
    public function list()
    {
        ...
        $builder = $builder = $this->carBrand->builder();
        $builder->select('c1.*, COUNT(DISTINCT c2.code_no) AS cnt');
        $builder->from('car_brands AS c1');
        $builder->join('car_brands AS c2', 'c1.code_no = c2.parent_code_no', 'left');
        ...
        return view('admin/car_brands/list', $data);
    }
    ...
    public function change_order()
    {
        ...
        if(is_array($c_idx)){
            for($i = 0; $i < count($c_idx); $i++) {
                $result = $this->carBrand->update($c_idx[$i], ["onum" => $onum[$i]]);
            }
            if($result) {
    ...
    public function write()
    {
        ...
        if(!empty($c_idx)){
            $record = $this->carBrand->where('c_idx', $c_idx)->first();
            $data["c_idx"] = $c_idx;
            ...
            $row_category = $this->carBrand->where('code_no', $s_parent_code_no)->findAll();
            $dp = $row_category[0]["depth"] ?? 0;
            $depth = $dp + 1;
            $query = $this->carBrand->select('IFNULL(MAX(code_no), "' . $s_parent_code_no . '00") + 1 AS code_no')
        ...
        return view('admin/car_brands/write', $data);
    }
    ...
    public function save()
    {
        ...
        if(!empty($c_idx)){
            $result = $this->carBrand->update($c_idx, $data);
            $carBrand = $this->carBrand->find($c_idx);
            if(!empty($del_img) && $del_img == 'Y'){
                if($carBrand["ufile1"]){
                    $filePath = $uploadPath . $carBrand["ufile1"];
                    if(file_exists($filePath)){
                        unlink($filePath);
                    }
                    $this->carBrand->update($c_idx, [
                        "ufile1" => "",
                        "rfile1" => ""
                    ]);
                }
            ...
            if($file->isValid() && !$file->hasMoved()){
                ...
                $this->carBrand->update($c_idx, $data_file);
            }
            ...
            if($file->isValid() && !$file->hasMoved()){
                ...
                $this->carBrand->update($id, $data_file);
            }
